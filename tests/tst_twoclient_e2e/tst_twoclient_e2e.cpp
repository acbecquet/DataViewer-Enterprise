...
private slots:
    ...
    void cellEditOnClientAReachesClientBLive() {
        ...
        // Seed a data row directly (mirrors tst_livesync fixture). v3 Phase 3d
        // (plan Task 4): via the dual-mode helper - legal both sides of the
        // cutover; post-cutover there is no data-row id and the helper returns
        // 0 (the per-cell path keys on (sample, metric, sort_order) instead).
        qint64 sampleId = -1;
        qint64 dataRowId = -1;
        {
            ClientScope scope(A.appName);
            ...
            QSqlDatabase seedDb = A.pg->queryDb();
            sampleId = DVE::TestSeed::seedSample(
                seedDb, testId,
                { { QStringLiteral("sample_name"), QStringLiteral("A") } });
            QVERIFY(sampleId > 0);
            dataRowId = DVE::TestSeed::seedDataRow(
                seedDb, sampleId, 0,
                { { QStringLiteral("draw_pressure"), 0.0 } });
            QVERIFY(dataRowId >= 0);
        }

        // Drain any NOTIFYs generated by the seed inserts so the spy only
        // captures the upcoming per-cell event.
        QCoreApplication::processEvents(QEventLoop::AllEvents, 200);
        B.cellChangedSpy->clear();

        // Client A: commit a TPM measurement via LiveSync (v3 Phase 3d: the
        // per-cell path is dve_commit_measurement, keyed by natural identity).
        {
            ClientScope scope(A.appName);
            QVERIFY(A.ls->commitMeasurement(sampleId, "draw_pressure",
                                            /*sortOrder=*/0, 3.14));
        }

        // Client B: poll up to 1.5 s for the value to land - read through the
        // measurements join, which is the write's target in both worlds.
        ...
        while (timer.elapsed() < 1500) {
            ...
            if (q.exec(QString(
                    ...
                    "WHERE m.sample_id=%1 AND md.key='draw_pressure' "
                    "AND m.sort_order=0").arg(sampleId)) && q.next()
        ...
        // v3 Phase 3d / index decision D3: measurements carry NO notify
        // trigger - one whole-file save would otherwise fan ~13k NOTIFYs to
        // every client (the DV-23 storm class) for zero UI benefit, since the
        // 2026-07-30 audit proved TPM row NOTIFYs drive no UI. So client B
        // must observe the VALUE (above) while its cellChanged spy stays
        // SILENT for this commit. The sensory json path, which does notify,
        // is covered by the sensory scenarios and tst_livesync.
        QCoreApplication::processEvents(QEventLoop::AllEvents, 300);
        QCOMPARE(B.cellChangedSpy->count(), 0);
