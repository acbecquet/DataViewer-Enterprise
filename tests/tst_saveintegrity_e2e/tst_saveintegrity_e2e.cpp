...
private slots:
    ...
    // ======================================================================
    // W2 (2026-08-27 smoke finding) - RESOLVED AS NOT-A-BUG. The two
    // identical-path files rows the owner's v2.10.6 smoke produced are the F6
    // versioned re-add DESIGN (v2.5.0): a fresh load FROM DISK deliberately
    // mints a NEW versioned row (UNIQUE(file_path, added_at)); in-session
    // saves UPDATE that row by id; loadFileByPath resolves the newest. The
    // contract is pinned by scenario5_tpmReAddAndContinue above and the F6
    // comment at MainWindow.cpp onFileLoaded (deliberately does NOT adopt by
    // path). A choke-point path-adoption "fix" was tried here and REVERTED -
    // it broke scenario5's third leg, which is the feature itself.
