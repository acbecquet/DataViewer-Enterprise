...
**Priority:** P2 · **Effort:** XL (multi-phase) · **Status:** **RESOLVED — OPTION B (owner 2026-06-24); spike DONE.**

> **⚠ AUTHORITATIVE — see the sub-spec `docs/superpowers/specs/2026-06-24-DATAVIEWER-6-MS4-optionB-spike.md`.** Owner chose **Option B** (remove the TPM table; in-app TPM editing/sync was never used — TPM data is authored in Excel + uploaded to view plots). The spike + completeness critic **supersede the Option-A/B framing and the Acceptance/Open-questions below.** Key corrections the notes below get wrong: (1) the **"View/Layout" ribbon tab is already dead code** (no `addTab("View")` — `setupRibbon:601-604`); there is nothing to "retire." (2) `onPropCellChanged` also writes `m_dataTable` (`:2168-2187`) → must be **reworked**, not just kept. (3) the sample-nav bar must be **re-parented** (it survives), not deleted. (4) **three LIVE cleanup bugs** independent of the teardown — multi-file Full report uses the wrong file's exclusions (`buildCleanedFile` hardcodes `m_currentFileIndex` `:6914`), file-close corrupts exclusion keys (`:2511-2518`), reopening the dialog discards manual exclusions (`:205-208`) — **fixed FIRST (Phase 2b.0)**. Plus a new **"View Raw Data"** Tools button (opens the Navigator-selected active file's Excel; Detailed needs a new `sourceFilePath` field) and a **Cleanup robustness pass**. 6-phase plan (2b.0–2b.5) + owner decisions in the sub-spec §9.
...
2. **KEEP Ctrl+U (owner decision 2026-06-24)** — it is a useful manual backup; do NOT retire it. This locks MS-7 to the **minimal** scope (keep both write paths). No comment/tooltip changes needed.
...
> 3. **Privilege:** the only DB role is the **superuser** `dataviewer_app`; a shared passcode over it gates full-DB write. → A **dedicated least-privilege `sensory_web` role** (INSERT/UPDATE/SELECT on `sensory_sessions` only; `REVOKE` on `dve_commit_cell*`) + an **anonymous, frictionless UX** (no user password/token; owner decision 2026-06-24) with backend-only controls (validation + invisible proxy rate-limit).
>
> Owner-locked: reachable from **anywhere** (internet + TLS; anonymous form, no user password — office-WiFi REJECTED), **online-only**, **5-metric Sensory**. Plus add a stable per-sample `sample_uid` for idempotency. See sub-spec §10 for the owner decisions (all resolved 2026-06-24).
...
- *(RESOLVED — sub-spec §1/§10.)* Internet-reachable via reverse proxy + TLS; **anonymous form, no user password** (office-WiFi rejected). All auth/abuse control is backend.
...
| R-K | Network-exposure decision stalls MS-8 | MS-8 | Med | Low | Spike DONE — network decided: internet via reverse proxy + TLS; anonymous form (no user password); office-WiFi rejected |
...
| R-N | Web service holding the superuser `dataviewer_app` role → a single service bug = full prod-DB compromise | MS-8 | Critical | Med | Dedicated least-privilege `sensory_web` role (INSERT/UPDATE/SELECT on sensory_sessions only; REVOKE on dve_commit_cell*); anonymous UX with backend-only validation + invisible per-IP rate-limit; never call generic commit fns |
