...
### Phase 2b — TPM table → Plot + Notes panel (Option B teardown; see the MS-4 Option-B sub-spec)

> **RESOLVED 2026-06-24: OPTION B.** Tasks 2.4–2.7 below (the Option-A additive version) are **SUPERSEDED** by the 6-phase sequence in `docs/superpowers/specs/2026-06-24-DATAVIEWER-6-MS4-optionB-spike.md` §2: **2b.0** fix the 3 live cleanup bugs (GAP-A/B/F) + tests → **2b.1** build the Notes panel additively (cleaned-sample-fed) behind the hidden table → **2b.2** remove the table surface atomically (incl. reworking `onPropCellChanged`, re-parenting the sample-nav, deleting the dead `buildViewTab`/delegates/`tst_cellfocusdelegate`) → **2b.3** "View Raw Data" button → **2b.4** raw/SOP empty-state → **2b.5** verify. Owner decisions (Detailed `sourceFilePath`; persist exclusions?) in sub-spec §9.
...
- [x] **Task 4.3 → MS-7 — DROPPED (owner 2026-06-24): KEEP Ctrl+U** as a useful manual backup. No retirement; comments/tooltips unchanged. This locks MS-7 to the **minimal** scope (keep both write paths — fix only the `oil_smell_liking` session-field merge gap in Task 4.2).
