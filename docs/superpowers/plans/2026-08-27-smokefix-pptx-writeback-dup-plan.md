...
## EXECUTION RECORD (2026-08-27, same session as the plan)

Tasks 1-5 executed inline (executing-plans); deviations from the written plan, each deliberate:

- **Task 3 sensory-deck step replaced.** No test renders a real sensory deck headless BY DESIGN (the QPixmap deadlock class tst_reportgenerator documents). All sensory text flows through the same two emitters (`makeTableCell`/`makeTextBox`) whose choke point `escapeXml` now strips illegal chars, and the harness proves deck-level validity with hostile text (incl. an embedded 0x01). The owner's PowerPoint open of a regenerated sensory deck in the re-smoke is the final W4 arbiter.
- **Task 4 resolved NOT-A-BUG and the fix REVERTED.** The duplicate `files` rows are the F6 versioned re-add design (v2.5.0): scenario5's third leg IS the feature, and MainWindow's F6 comment deliberately refuses path adoption on disk loads. The choke-point adoption broke scenario5; scenario26 was replaced by a design-pointer comment. Commits a9c766b (RED) + 07d4d83 (re-scope) tell the story.
- **Task 5 headers script deleted, not rewritten.** `kWriteHeaders` was 8 fixed cell writes with the same per-cell semantics as `excelWriteCellsScript`; the header write now builds `ExcelCellWrite`s and rides the one surgical script (DRY, one less script to drift).
- **Task 5 addition beyond the plan:** the surgical scripts restore the original raw root start-tag after ElementTree serialization - ET drops xmlns declarations that appear only in attribute VALUES (`mc:Ignorable="x14ac"`), which would itself have been a repair trigger on real template sheets. Also `DVE_READER_NO_COM` (test-only env) forces the openpyxl fallback so the stripped-cache detection is deterministic on Office machines.
- **tst_excelwritepayload byte-identity guards retired** (they pinned the openpyxl scripts verbatim - the bug itself); replaced with structural tripwires: surgical (`zipfile.ZipFile` present), atomic (`os.replace` + `.dve_tmp`), `openpyxl` never returns to the write path, `fullCalcOnLoad` never disappears.

Green at record time: tst_pptxvalidate 4/0 (NEW), tst_excelsurgery 8/0 (NEW), tst_xmlbuilder 13/0, tst_sheetprocessors 15/0, tst_excelreader 17/0, tst_excelwritepayload 13/0, tst_saveintegrity_e2e 28/0, tst_pptxwriter 18/0, tst_reportgenerator 15/0, tst_reportlayout 8/0. Task 6 (full suite + corpus gates + v2.10.7 installer + re-smoke) in flight.
...
- [x] **Step 1: Write the validator script** `tests/tst_pptxvalidate/validate_pptx.py`. Stdlib only. Exit 0 = clean, exit 1 = violations (one per line on stdout, prefixed `VIOLATION:`).
...
- [x] **Step 3: Register the suite** in `tests/tests.pro` `SUBDIRS`, `.pro` file mirrors a neighbor suite (`tst_reportgenerator/tst_reportgenerator.pro`) with `SOURCES += tst_pptxvalidate.cpp` plus the reporting/utils sources that suite links.

- [x] **Step 4: Build + run, verify RED for the RIGHT reason.**
...
- [x] **Step 5: Commit the RED harness** (`git add tests/tst_pptxvalidate tests/tests.pro && git commit -m "test(pptx): semantic OOXML validator harness - RED on tcPr order + control chars"`).
...
- [x] **Step 1: Reorder `tcPr` children** - lines first, fill after:
...
- [x] **Step 2: Re-run the harness.** Fix every remaining `VIOLATION:` it prints the same way (each is a precise pointer: part + element + rule). Known candidates from the code read, if reported: duplicate `cNvPr` ids across shapes on one slide (fix = single per-slide id counter in the slide builders), unexpected children the ORDERS table flags. Do NOT touch the control-char violation yet (Task 3 owns it) - temporarily drop the `\x01` row from the Task 1 fixture if it is the only red left, then restore it in Task 3.
- [x] **Step 3: Re-run `tst_pptxwriter`, `tst_reportgenerator`, `tst_reportlayout`** - update any assertions that byte-match the old cell XML.
- [x] **Step 4: Commit** (`fix(pptx): emit tcPr children in schema order - clears the chronic PowerPoint repair prompt`).
...
- [x] **Step 1: Failing unit test** in `tst_xmlbuilder`:
...
- [x] **Step 2: Implement** - extend the switch's default branch:
...
- [x] **Step 1: Failing E2E** (exact save API: whatever `scenario17` calls - `m_db->saveFile(...)` - copy its call shape):
...
- [x] **Step 2: Implement adoption** in `persistFileCore`, immediately before the `if (result.id != -1 && result.version > 0)` branch:
...
- [x] **Step 3: GREEN + regression.** Run `tst_saveintegrity_e2e` fully, then `tst_databasemanager` (its destructive/OCC slots exercise the same branch selection). All green.
- [x] **Step 4: Commit** (`fix(db): adopt the existing files row by path on id-less saves - no more duplicate file rows on reopen`).
...
- [x] **Step 1: Fixture generator** `tests/tst_excelsurgery/make_formula_fixture.py` - builds `fixture.xlsx` from raw parts (total control over caches; openpyxl cannot write cached values):
...
- [x] **Step 2: Checker script** `tests/tst_excelsurgery/check_surgery.py` - asserts post-surgery invariants, exits nonzero with a message on any failure:
...
- [x] **Step 10: Gate the fabrication.** `SheetProcessors.cpp:148` - wrap the existing fill with the detection (behavior byte-identical for healthy files):
...
- [x] **Step 11: Surface + block.** In `MainWindow::loadFile`'s completion path (find where the parsed `FileResult` lands and the status bar is set): when any sheet has `strippedFormulaCells > 0`, show a `QMessageBox::warning` - "This workbook's computed values are missing (it was last saved by a tool that strips them). Open it in Excel, let it recalculate, save, then reload. Values shown may be incomplete; saving this file to the database is disabled." - and record the file as save-blocked (member set keyed by file path; the DB-save entry points skip blocked files with a status-bar notice). Search every save entry point per the cover-every-load-path lesson: the manual save action, autosave/close-time save, and the sync indicator path.
- [x] **Step 12: Tests for the gate:** in the surgery suite, run the ORIGINAL openpyxl-destroyed fixture (make one by running the OLD behavior: openpyxl load+save in the fixture generator with `--strip` flag) through `DVE::ExcelReader` + `DataProcessor::processFile` and assert (a) `strippedFormulaCells > 0` on the sheet, (b) puffs zeros were NOT extrapolated. Then run the corpus gates (`tst_v3shadow`, `tst_v3roundtrip` with `DVE_TEST_CORPUS_DIR="C:/Users/S1134987/Documents/Python/DataViewer Dev/DB Data"`) - green, because no corpus file trips the detector (~20 min each, expected).
- [x] **Step 13: Commit** (`fix(excel): reader detects cache-stripped workbooks - warn + block DB save instead of fabricating puffs`).
