# Lessons Learned

## 2026-03-19: Sensory layout should be horizontal, not vertical
- **Mistake:** Set sensory panel splitter to `Qt::Vertical` (cards top, chart bottom) matching the TPM layout
- **Correction:** Sensory mode uses a **horizontal** layout — cards on the left, radar chart on the right. This is intentionally different from TPM mode (table top, plot bottom).
- **Rule:** Don't assume new UI modes should mirror existing layout orientations. When the user says "place data and plot in the space where table and chart are," the spatial arrangement can still differ.

## 2026-05-11: QSqlDatabase reference release on Windows
- **Discovery:** Calling `db.close()` alone is insufficient on Windows — the file handle isn't released until `QSqlDatabase::removeDatabase(name)` is called. When the Qt documentation warns "connection still in use", assigning `db = QSqlDatabase()` first prevents the warning.
- **Applied in:** `PostgresConnection::close()`, `PostgresConnection::open()` (connection cycling), and `MigrationTool::finalizeSource()` (SQLite close after reading source).
- **Rule:** Always call `QSqlDatabase::removeDatabase()` when closing a database for the last time, or clear the object reference first.

## 2026-05-11: QTemporaryFile blocks QFile::rename on Windows
- **Discovery:** Even after explicit `close()`, `QTemporaryFile` retains a file-share flag that prevents `QFile::rename()` on the same object. The file is not fully released to other processes until it goes out of scope.
- **Applied in:** `tst_migrationtool` — tests that rename files should use a plain path under `QDir::tempPath()` instead of `QTemporaryFile`.
- **Rule:** For tests that need atomic file operations (create temp, rename to final), avoid `QTemporaryFile`; use explicit temp paths and manual cleanup.

## 2026-05-11: MIP/AIP source-file mitigation
- **Discovery:** Every new `.cpp`/`.h` file in this repo may inherit Microsoft Information Protection (MIP) sensitivity labels at rest, causing build tools (`g++`, `head`, `cat`) to see ciphertext (`%TSD-Header-###%...`). The Python interpreter is on the MIP allowlist and reads plaintext.
- **Applied in:** All file creation in Plan A — write new source files via Python's delete-and-rewrite pattern (see CLAUDE.md section "File creation convention"), not via the Write tool. Before any C++ build, run `python tools/decrypt_via_copy.py --apply` to strip labels.
- **Rule:** New `.cpp`/`.h` files must be written via Python. Subagents that produce source code must include the deletion pattern. The decrypt script is idempotent and runs as a build pre-step until labels stabilize.

## 2026-05-11: postgres:16 doesn't bundle pg_cron
- **Discovery:** The official Docker image for `postgres:16` does not include the `pg_cron` extension. Container startup fails on `CREATE EXTENSION pg_cron` with "could not access file pg_cron".
- **Applied in:** `deploy/postgres/Dockerfile` — derived image installs `postgresql-16-cron` from the PostgreSQL Development Group (PGDG) repository before initializing the database.
- **Rule:** When using a postgres Docker image, always derive a custom image for any non-bundled extensions (`pg_cron`, `pg_partman`, etc.). The Dockerfile pattern is: `FROM postgres:16 && apt update && apt install -y postgresql-16-<extension> && ...`.
## 2026-05-28: VBA/COM worksheet copy — capture by name-diff, never by position/ActiveSheet
- **Discovery:** Seeding the post-upload reset snapshots failed two ways that silently corrupted the workbook. (1) `Worksheet.Copy After:=Worksheets(Count)` when the last sheet is **very-hidden** (`_Template_Master`) misplaces the copy, so capturing the new sheet by position grabbed a pre-existing sheet and `snap.Name = ...` renamed it 12 times (only `_Template_11` survived; `_Template_Master` was clobbered). (2) In pywin32, `ws.Copy(After=anchor)` as a **named** arg doesn't bind in late-bound COM and silently spawns a throwaway workbook (0 sheets added to the target).
- **Applied in:** `excel-sidecar/DataViewerUpload.bas` — `AddSnapshotFrom` and `RestoreSheetFromTemplate` copy after/before a **visible** anchor (the "DataViewer Upload" sheet / the live sheet), find the new sheet by **name-diff** (record sheet names before the copy), and never `Delete` the original until the replacement is confirmed present. Recovery COM scripts pass the anchor **positionally** as `Before` (`ws.Copy(beforeSheet)`), not `After=`.
- **Rule:** When copying sheets in VBA/COM: (a) never anchor Before/After on a very-hidden sheet; (b) capture the new sheet by name-diff, not `ActiveSheet` or `Worksheets(Count)`; (c) make any replace transactional — copy first, confirm, then delete; (d) in pywin32 pass `Copy`/`Move` anchors positionally, not as `After=`. Separately: an over-strict guard (snapshot `A1` must equal the live sheet's `A1`) false-skipped a sheet whose template title legitimately differed ("Lifetime Test 2") — prefer a robust structural invariant over brittle equality.

## 2026-05-29: Never reuse a color within a plot
- **Discovery:** Plot palettes were small fixed arrays indexed with `% N`, so colors repeated once the series count exceeded the palette — the Lifetime comparison chart read as "only blue and orange," and 7+ files wrapped hues back to blue. The same `% N` flaw existed in `PlotEngine`, `PlotWidget`, and `RadarChartWidget`.
- **Applied in:** New `AppTheme::seriesColor/seriesColors/shade` (single source of truth, reusing the radar chart's already-vetted no-yellow palette + golden-angle generation past 20). All report charts, GUI plots, and radar overlays route through it; the `ReportGenerator` comparison chart stays grouped by file via distinct base hues + `shade()` per sample.
- **Rule:** Never `palette[i % N]` in a plot — size the palette to the element count via `AppTheme::seriesColors(n)` / `seriesColor(i)` (or `shade()` for grouped charts). No yellow (hue ≈ 45°–70°); keep series saturated and mid-dark — yellow and pale/light colors wash out on projectors.

## 2026-05-29: Repurposing a positional column — audit EVERY consumer, not just the obvious ones
- **Discovery:** Making per-row data-table column index 4 dual-purpose (Resistance double ↔ Puffing Regime string, v2.2.1) required touching far more than the read/write/display path. A non-obvious consumer — the live multi-user conflict "take their value" tooltip (`MainWindow::handleRemoteRowChange`) — used a hardcoded 12-column positional `SELECT ... resistance ...` and stamped values by visual column index, so accepting a remote regime edit silently overwrote the regime with resistance. Per-task reviews missed it (it wasn't in any task's scope); only the whole-feature integration review caught it.
- **Applied in:** `liveColumnForDataCol`/`dataColForLiveColumn` (sheet-aware col↔DB-column map) cover the LiveSync paths; `handleRemoteRowChange` now fetches `puffing_regime` and stamps it into col 4 when `sheet->hasPerRowRegime`. `DataRow` keeps BOTH `resistance` and `puffingRegime`; the per-sheet `SheetResult::hasPerRowRegime` flag drives all branches.
- **Rule:** When a positional column/index changes meaning, grep the WHOLE repo for EVERY positional consumer of that index/field/DB-column (SELECT column-lists, `value(N)`, index-based stamps, write-back arithmetic, conflict/tooltip paths), not just the obvious populate/edit/save. A final cross-task integration review is where these between-the-cracks consumers surface — do it.

## 2026-06-05: A spec-conformance audit can't catch a spec that doesn't match intent
- **Mistake:** Plan C (auto-recovery) was brainstormed, built, audited (5 parallel auditors), and shipped as v2.3.0 scoped as **crash-only** recovery — it deliberately cleared the snapshot on a clean/declined close and only offered to restore after a crash/update. The user's very first real test ("opened files, closed, said *don't save*, then nothing to reload") exposed that they actually wanted **restore-my-last-session**: persist the snapshot across *any* close and auto-offer to reopen (files + unsaved edits) on *every* launch. The deep audit passed because it verified the code against the (mis-scoped) spec — conformance ≠ correctness of intent.
- **Root cause:** Brainstorming locked "clear on clean close" + "crash detection" as decisions without surfacing the one UX fork that actually mattered: *does a normal/declined close discard the session, or should it always be restorable next launch?* That question was never put to the user.
- **Applied in:** v2.3.1 — `closeEvent` finalizes via `flushNow(true)` instead of `clear()`; the launch prompt auto-offers on any prior session and **lists the item names**; restore includes unsaved edits; DB-loaded files are snapshotted; "No" keeps the session retrievable (no accidental-dismiss data loss).
- **Rule:** For any "don't lose my work" / recovery / session-restore feature, explicitly ask the user the close-behavior fork up front (clean/declined close = discard vs. always-restorable?). And never treat a passing spec-audit as proof the feature is right — for user-facing behavior, verify against the user's actual goal with a quick real run (or have them try it) *before* calling it done, because an audit only proves the code matches the spec, not that the spec matches what they wanted.

## 2026-05-29: Ephemeral test Postgres applies init.sql's base block only — not migrations
- **Discovery:** `tests/start-test-postgres.ps1` applies only the `BEGIN..COMMIT` block of `init.sql`, NOT the files in `deploy/postgres/migrations/`. So DB tests that exercise migration-only schema (e.g. sensory header presets from `2026-05-20-sensory-header-presets.sql`) FAIL against the ephemeral container (`saveSensoryHeaderPresets`/`saveSensoryLayout` return FALSE) — a pre-existing, environmental failure unrelated to whatever feature is under test.
- **Applied in:** The v2.2.1 `data_rows.puffing_regime` column was added to BOTH `init.sql` (base block) AND a migration file — so the ephemeral container has the column and `perRowRegime_roundTrip_newAndOld` passes. The migration is only for already-deployed DBs.
- **Rule:** A new column/table must go in `init.sql`'s `BEGIN..COMMIT` block (fresh installs + the test container both read it), with a separate migration file ONLY for upgrading deployed DBs. When triaging `tst_databasemanager` failures against the local container, first check whether the failing feature's schema lives only in a migration.

## 2026-06-10: A sync heuristic must never DISCARD a save (v2.4.0 data-loss postmortem)
- **Mistake:** Three v2.4.0 mechanisms each assumed "LiveSync already delivered every edit to the DB": (1) `VersionMismatch/RowDeleted => skipped -- already up to date via LiveSync` dropped the dirty flag without writing; (2) the DB-authoritative score merge unconditionally took DB values for score cells; (3) flushNowAndWait()'s timeout was unchecked. When LiveSync was gapped (id<=0 sessions never stream; worker connection broken with 26000 errors and no reconnect), each mechanism actively destroyed the user's in-memory edits -- production log showed 23 silent save-discards in 3 days ("file reverted the next day").
- **Correction (v2.4.1):** fresh-version OCC inside the write cores (row-level last-writer-wins by design); callers retry/reinsert and NEVER clear dirty on failure; panels track dirty cells and the merge keeps in-memory values for cells the user touched; worker reconnects; failures surface in the sync indicator.
- **Rule:** Any "skip this write, another component already did it" heuristic must VERIFY, not assume. Cross-component freshness assumptions need arbitration data (dirty set, timestamps, versions). A save path may fail loudly or retry -- it may never silently drop user data.

## 2026-06-10: jsonb cell streaming must preserve types (the real "reset to 5")
- **Discovery:** `dve_commit_cell_json` stored every streamed value as `to_jsonb($2::text)` -- numeric scores landed as JSON *strings*. `QJsonValue::toDouble(5.0)` returns the DEFAULT for strings, so every live-streamed score read back as 5.0 on the next fresh load. This had corrupted rows since v2.0.2 and was invisible to tests because the test helper simulated commits with `::double precision` (a number) instead of the production text path.
- **Correction:** numeric-regex CASE in the stored function (numbers stored as numbers), tolerant `jsonToDouble` reader (repairs historical string rows on read), ensureSchema CREATE-OR-REPLACE heal. Also: CREATE OR REPLACE with a different arity creates an OVERLOAD, not a replacement -- the live DB had both a 6-arg and 7-arg `dve_commit_cell_json`; only pg_get_functiondef/pronargs reveals which one the app actually calls.
- **Rule:** Test helpers must replicate the production write path byte-for-byte (same stored fn, same casts). When healing stored functions, match the exact arity the client binds, and check for overload shadows.

## 2026-06-10: Qt Test totals shrinking == stale binary, not lost tests
- **Discovery:** Suite totals regressed (64->63, 17->15) across verification runs. Cause: the test runner writes per-suite logs to a fixed `%TEMP%\<suite>.txt`, and polling mid-build reads the PREVIOUS run's file; separately, a build that fails/lags for one suite leaves an old exe that runs happily with old counts.
- **Rule:** Trust only totals from a log you watched get written: run the freshly-built suite exe directly with `-o <file>,txt`, or verify the exe mtime postdates your build. Before concluding tests were lost, grep the source for the test names.

## 2026-06-10: Never hard-block the user; auto-resolve or offer options (product directive)
- **Directive (user, verbatim intent):** "I don't want to stop the user experience in their tracks at any point, just provide guidance and ALWAYS provide options." Applied: duplicate/renamed session names auto-suffix `_1/_2/_3` silently (status-bar notice, no modal); closing an unnamed session offers Name It Now / Discard Session / Cancel instead of a forced-naming OK box; TPM close-persist failure offers Retry / Close Anyway / Cancel; TPM re-adds of the same file mint a new timestamped DB version instead of overwriting.
- **Rule:** For any new validation/conflict surface in this app: first try silent self-resolution (suffix, version, merge); if user input is genuinely required, a dialog must offer at least [do it for me] / [destructive-but-explicit] / [cancel] -- an OK-only blocker is a design bug.

## 2026-06-18: Database Browser opens from the Home ribbon, not Tools (SP4-T3 visual loop)
- **Mistake:** Told the user to open the Database Browser via "Tools -> Database Browser". It is the "Database" large button in the **Database group on the Home ribbon** (`MainWindow.cpp:641`: `dbBtn` -> `onOpenDatabaseBrowser` -> `DatabaseBrowserDialog`). Has been on Home since v2.
- **Rule:** The Home ribbon (not Tools) hosts the file/database actions: New File, Load File, Close, SOPs, **Database**, Refresh Snapshot, Sensory, Detailed Sensory, Images. Check the actual `tab->addGroup(...)->addLargeButton(...)` wiring before telling the user where a feature lives.

## 2026-06-18: Running the bare build for a visual check needs windeployqt + libpq + resources (SP4-T3 visual loop)
- **Discovery:** `build\release\DataViewer.exe` is the raw linker output -- no Qt runtime DLLs (Qt6Sql.dll missing -> "code execution cannot proceed"), and `resourcePath()` finds no `resources/` so ribbon icons render blank. `windeployqt` deploys Qt's own DLLs/plugins but NOT the app's `resources/` PNGs (the Inno installer does that), and the QPSQL driver needs `vendor/libpq-16/*.dll` beside the exe.
- **Rule:** To run the freshly-built exe for a visual check WITHOUT reinstalling: `windeployqt --release --compiler-runtime build\release\DataViewer.exe` + `cp vendor/libpq-16/*.dll build/release/` + `cp -r resources build/release/resources`, then launch with `LOCALAPPDATA` pointed at a sandbox `db.conf` (test container, never prod). Blank icons / missing DLLs in such a run are deployment artifacts, NOT regressions -- they do not ship.

## 2026-06-18: DB-browser triage filter -- parent (group) rows leave Version/Health blank (SP4-T3, owner-approved)
- **Decision:** In the Sensory/Detailed tabs the tree is grouped (parent = test title, children = per-tester records). Version/Health are set on the CHILD rows only; parent rows show blank cells in those two columns. Owner reviewed against seeded data and approved ("both look right"). Era/health is a per-record property, so a parent (spanning testers/eras) has no single correct value; a roll-up was considered and declined for now.
- **Rule:** If a future ask wants parents to summarize child health (e.g. worst-of), it is a small change in `populateSensoryTree`/`populateDetailedSensoryTree` -- but the current intentional behavior is blank-on-parent.

## 2026-06-22: Sprints are scoped by the minor version -- don't resurface closed-sprint plans
- **Mistake:** Listed "Plan C -- auto-recovery + crash snapshot" as the #1 *pending* next task, pulled from an 18-day-old memory that said "NEXT BIG TASK / surface FRONT AND CENTER." Plan C had actually shipped (v2.3.0 crash-only -> v2.3.1 restore-last-session -> v2.4.4 R5 MIP hardening); the `RecoveryManager` (`src/utils/RecoveryManager.{h,cpp}`) is live in the product. The owner had to correct me.
- **Correction:** The owner's workflow delimits sprints by the **minor version** (`y` in `x.y.z`). Each minor is one sprint; internal patch builds wrap into the next deployable minor (the v2.4.x batch -> deployable v2.5.0). When a sprint's line has shipped, its plans/specs are CLOSED.
- **Rule:** When planning or listing next tasks, only treat the CURRENT minor's plans as active. Never surface a prior-minor plan as pending without first verifying it against current code + git -- stale "PENDING" memories from old sprints lie. (Deleted the stale Plan C memory; recorded the rule in `feedback_sprint_by_minor_version.md`.)

## 2026-06-23: A "file open" bug has THREE entry points -- repro the exact one before fixing
- **Mistake:** "Opening an already-open file creates a `_1` duplicate that splits the DB." I assumed the TPM path (`MainWindow::routeFile`->`loadFile`->`m_loadedFiles`) and fixed only that (v2.4.11). The owner's actual repro was a **sensory** file, which routes to `SensoryPanel::loadFile` (a separate `m_sessions` list with no dedup) -- so the duplicate persisted. The `dataviewer.log` even showed the sensory-specific signal (`tryWriteSensorySessionAutoSuffix ... "New test sensory_1"`) which I had dismissed earlier as "a separate intended thing."
- **Correction:** There are THREE file-open paths, each with its own session list + dedup: TPM (`routeFile`->`loadFile`->`m_loadedFiles`, keyed by normalized file path), Sensory (`SensoryPanel::loadFile`->`m_sessions`), Detailed (`DetailedSensoryPanel::loadFile`->`m_sessions`). Sensory/detailed dedup by the DB natural key (`session_name`+`tester`+`date`), NOT by `sourceFilePath` (which exists on the struct but is never set and isn't persisted across the recovery/DB JSON round-trip, so recovered sessions carry none). Fixed all three (v2.4.11 TPM, v2.4.12 sensory+detailed).
- **Rule:** For a UI bug with multiple entry points (file types, modes, panels), enumerate them and confirm WHICH the user hit -- the on-screen panel + the log signals tell you -- before coding a fix; don't assume the first path. And don't dismiss a concrete log signal (the `_1` was sensory) as unrelated.
- **Follow-up (cost me 4 build cycles):** "open a file" actually meant "load a session from the DATABASE BROWSER" -- a 4th entry point (`SensoryPanel::loadSessions`, NOT loadFile). I only found it by reading `dataviewer.log` and noticing my `[sensory-load]` diagnostic NEVER fired (= the function I kept fixing wasn't even being called). **Meta-rule: after a fix fails to work, do NOT rebuild another guess. Instrument, have the user reproduce, and READ THE LOG / ask for the exact UI action to confirm the code path is actually executed before iterating again.** A `git grep` for every caller of the relevant panel method (`loadSessions`, `loadFile`, `newSession`, recovery restore) up front would have surfaced all entry points at once. (DB loads are best deduped by row `id`, the reliable identity -- not file path or natural key.)
- **Closing (v2.4.15):** Owner then made it a standing rule: *"always cover every path when I'm describing loading -- assume all paths and check each one"* (saved as the `cover-every-load-path` memory). Applied it PROACTIVELY and found the SAME dedup gap in two more entry points: `SensoryPanel::loadFiles` (the multi-select "Load Sensory Files" button + its Excel/JSON helpers) and `DetailedSensoryPanel::loadFiles` -- both blindly appended. Fixed both and consolidated the sensory dedup into one shared `reconcileNewlyLoaded(firstNew)` helper so `loadFile`/`loadFiles`/`loadSessions` can't drift apart again (the root cause of this whole saga was exactly that drift). The full set, now all deduped: TPM `routeFile`; sensory `loadFile` / `loadFiles` / `loadSessions`; detailed `loadFile` / `loadFiles` (detailed `loadSessions` REPLACES the list, so it structurally can't dup).

## 2026-06-23: Show the progress bar from the START of a blocking op, not partway
- **Bug (owner):** Close -> "Save All" after an edit froze the window for ~2s with no indicator, THEN the snapshot "Saving offline copy..." bar appeared for the rest. The dead gap was the DB save (`onUpdateDatabase`) inside `promptSaveDatabase`, which ran with NO dialog; only the later `regenerateSnapshotWithProgress` showed a bar.
- **Fix (v2.4.16):** one member `QProgressDialog* m_closeProgress` created at the START of the Save-All work (`makeBusyDialog`, busy/indeterminate), updated as it saves, and REUSED by `regenerateSnapshotWithProgress` (switched to determinate) so close shows ONE continuous bar; `closeEvent` tears it down via `closeProgressEnd()`. Audited the other blocking ops -- file load, report gen, snapshot refresh, DB repair already show live bars (status strip or modal dialog), so they were left as-is. Did NOT bolt a modal onto report gen: each report emits `reportFinished` -> `onReportFinished` pops its own modal box (per file in multi-file), which a second modal would fight.
- **Rule (owner directive; memory `progress-bars-for-blocking-ops`):** any operation that blocks the UI must show a progress indicator from the first heavy step -- never a frozen window before the bar. Construct + `show()` the dialog and `processEvents(ExcludeUserInputEvents)` BEFORE the work, not after phase one. A wait cursor is NOT a substitute for a hard-blocking op (it won't paint without an event pump, and pumping inside a deliberately-synchronous drain like `finishExcelWritesBlocking` risks re-dispatching the very async work it's draining).

## 2026-06-24: QDockWidget — maxWidth blocks re-docking; objectName required to persist
- **Bug (owner, v2.5.3):** After popping the Notes/Navigator docks out (floating), neither could be re-docked (drag-to-edge or the dock button did nothing); and the Navigator never remembered its float/position across runs while the Notes dock did.
- **Root cause 1 — re-dock:** a `setMaximumWidth()` (we capped both docks at 320) on a `QDockWidget` prevents `QMainWindow` from re-docking it after float — the dock-area layout can't create the placeholder drop-gap for a max-constrained widget, so the drop is silently rejected. A *minimum* width does NOT cause this; only a maximum does.
- **Root cause 2 — persistence:** `QMainWindow::saveState()`/`restoreState()` key docks by `objectName` and **silently skip** (with a runtime warning) any dock lacking one. Navigator had no `objectName`; Notes had `"notesDock"`.
- **Fix (v2.5.4):** removed the `setMaximumWidth` caps (control width via `resizeDocks()` / `restoreState()` instead; compact mode still uses a 32px max to collapse the icon strip, which is fine since re-dock isn't a use case there), and gave the Navigator `setObjectName("navigatorDock")`. When a dock gains/changes an objectName, old saved layouts can't match it — re-balance once (bump a QSettings flag) so the user sees a clean default, then their sizing persists.
- **Rule:** Never use `setMaximumWidth/Height` to size a floatable `QDockWidget` — it breaks re-docking; use `resizeDocks()`. Every dock that should persist MUST have a unique `setObjectName` set BEFORE `restoreState()` runs. When changing a dock's objectName, treat saved layouts as stale and re-apply a default once.

## 2026-06-25: Advisory input validation must NEVER gate persistence
- **Domain (owner, 2026-06-25):** the puffing-regime standard IS the CORESTA standard, but it is ALWAYS labeled parametrically as `60mL/3s/30s` — never as the word "CORESTA". So named strings like "CORESTA"/"CRM" are NOT valid regime values and *should* flag red. (They appear only in synthetic unit-test fixtures, not real data.) The red flag on non-parametric text is correct and aligned with the standard.
- **Mistake (v2.5.5, caught by an adversarial verify-workflow before owner saw it):** the per-row regime `QLineEdit` only emitted `cellEdited()` when the text matched the parametric format. The bug was NOT that it flagged non-standard text red (that's right) — it was that a red value was **silently DROPPED** (no `cellEdited` → field reverts on the next `rebuild()` with no error → pure data loss), and it disagreed with the still-permissive `RegimeComboDelegate` (the TPM-table editor of the SAME `puffingRegime` column).
- **Why blocking the save was wrong:** dropping a user's typed value with no feedback is worse than storing a non-canonical one — the display diverges from the model, the edit vanishes on refresh, and two editors of one field behave differently. (`RegimeUtils::regimeKey()` is also opaque — trim only, no numeric parse — so a non-standard value can't crash processing; it just forms its own group.)
- **Fix (v2.5.6):** emit unconditionally on `editingFinished`; the red border is a pure NON-BLOCKING hint (renamed `regimeLooksValid`→`regimeLooksStandard` so the name says it only styles, never gates). Widened the regex to tolerate spaces.
- **Rule:** "highlight red on bad input" means an ADVISORY visual cue, NOT a save gate — keep styling and persistence on separate paths. Never silently drop a user's typed value; if you must reject it, make the revert VISIBLE (snap back + a hint), don't just no-op the write.


## 2026-06-25: The TPM data grid was FIVE coupled surfaces, not just a table (DATAVIEWER-6 Option B)
- **Context:** MS-4 replaced the TPM-mode `m_dataTable` QTableWidget with the editable NotesStoryPanel. The naive read was "delete a table widget"; the reality was that the grid was the single surface for FIVE distinct behaviours, and removing it (Task 10) touched all of them.
- **The five surfaces:** (1) per-cell EDIT -> `onTableCellChanged` -> `DataRow` mutation -> recalc -> Excel write-back; (2) per-cell LIVE-COLLAB -> `onDataTableItemChanged` -> `LiveSync::commitCell`, plus inbound `onRemoteCellChanged/Focused/Blurred` painting remote edits via `CellFocusDelegate`; (3) OFFLINE capture -> `onTableCellChanged` queued a `PendingEdit`, replayed by `flushPendingEdits` on reconnect; (4) RAW/SOP-sheet rendering -> `displayCurrentSample` drew `rawRows` into the same widget; (5) cleanup STRIKETHROUGH -> excluded rows were greyed+struck in-grid. And a sixth, easily-missed coupling: `onPropCellChanged` (Sample-Properties, a MUST-KEEP slot sensory also uses) wrote the grid's calculated columns.
- **Option B kept every behaviour and re-pointed it at NotesStoryPanel:** edits route `NotesStoryPanel::cellEdited` -> `onStoryCellEdited` (same mutate->recalc->Excel+LiveSync path, indexed by data-row directly so there is no visible-row mapping); the row-deleted banner (T19) survived by SPLITTING `handleRemoteRowChange` (keep the file/sensory/detailed delete head, drop the data_rows decoration tail); raw/SOP became a panel hint (`showHint`, Task 12); strikethrough became the panel's readable "excluded" tint; `onPropCellChanged` now refreshes the panel instead of the grid.
- **What was genuinely deletable vs. what only LOOKED dead:** the two delegates (`CellFocusDelegate`/`RegimeComboDelegate`) + `tst_cellfocusdelegate`, the dead "View" tab (`buildViewTab` was never registered in `setupRibbon` -> all its slots were dead), `onAddRow`/`onRemoveRow` (no ribbon button wired them since v2.5.3), and the whole `PendingEdit`/`flushPendingEdits` unit (its ONLY producer was `onTableCellChanged`) were all safely removed. KEPT per owner: `m_liveSync`, `RemoteCellHelpers`+`tst_mainwindow_remotecell` (now compiled-but-unused-by-MainWindow, harmless), the whole-file `data_rows` DB save, sample-nav, `m_propTable`, `resolveUserName`.
- **Rule:** before deleting a UI widget, enumerate every BEHAVIOUR it hosts (edit / sync / offline / alt-render / decoration) and every OTHER slot that writes it (`onPropCellChanged` was the trap), then grep each candidate symbol across the WHOLE src/ tree to separate truly-orphaned code from look-alikes (the SQLite `drainPendingEdits` / sensory-panel `onRemoteCellChanged` are unrelated namesakes). Anchor on NAMES, never plan line numbers -- MainWindow churns constantly.


## 2026-06-25 -- MS-5 Sensory stopwatch (DATAVIEWER-13)

- Programmatic `QDoubleSpinBox::setValue()` re-fires `valueChanged`, so the per-card
  stopwatch reuses the ENTIRE existing puff-length persistence / LiveSync / report chain
  with zero new plumbing (no new struct field, serializer, migration, or DB write). When a
  field already round-trips end-to-end, drive it with `setValue()` instead of adding a
  parallel write path.
- A context-sensitive global hotkey (Space toggles the focused card's stopwatch but must
  NOT fire while typing) needs a `qApp` event filter with a focus-type guard
  (`qobject_cast<QLineEdit/QPlainTextEdit/QTextEdit/QAbstractSpinBox>`), NOT a `QShortcut`
  -- a Space `QShortcut` intercepts the key before text widgets and breaks typing
  everywhere. Guard on `!QApplication::activeModalWidget()` too so the rebind-capture
  dialog isn't hijacked.
- On this MIP machine the Read/Edit/Grep tools see working-tree source as ciphertext once
  MIP re-labels it (a short binary blob, not the real file). Edit source reliably by
  patching via the allowlisted Python interpreter (it reads plaintext) and let the build's
  decrypt step plaintext it for g++.


## 2026-06-25 -- MS-5 follow-up: active-card click focus + hotkey rebind feedback (DATAVIEWER-13)

- The active-sample outline + Space hotkey only followed focus into a card's
  focusable CHILD widgets. Clicking a label / the title / empty card space moves
  no Qt focus (those children are `Qt::NoFocus`), so `onAppFocusChanged` never
  fired and `m_focusedCard` stayed pinned to the previous card -- Space then only
  ever toggled the FIRST sample. Fix: give `SampleCard` `Qt::ClickFocus` + a
  `mousePressEvent` that `setFocus(Qt::MouseFocusReason)`, so ANY click on the
  card (child field, label, or background) routes through `cardOwning()` and makes
  it the active card. Rule: if a "selected element follows focus" feature must
  respond to clicking the whole element, the container needs click-focus, not just
  its inner widgets.
- A key-capture dialog that must be able to BIND Space/Enter cannot use focusable
  OK/Cancel buttons that consume those keys. Set the buttons `Qt::NoFocus` so all
  key events flow to the dialog's event filter, and confirm by MOUSE only. Echoing
  the captured key into a live label + an explicit "Set hotkey" confirm gives the
  user proof the right key registered (the owner's ask) without the Space-binds-vs-
  Space-clicks-OK conflict.

## 2026-06-25 -- MS-8 desktop data-safety + DV-15 canonical session_name (DATAVIEWER-11/15)

- `mergeSensoryPreservingDbScores` overlaid DB scores only across the index-overlap
  then returned the (shorter) in-memory `samples[]`, so a whole-session save
  SILENTLY DROPPED any sample the phone web form appended to the DB while the
  desktop held the session open (last-writer-wins re-adopts the shorter array).
  Fix is 3 lines: after the overlap loop, append `dbSamples[mem..db)` verbatim.
  The save path runs the merge INSIDE `tryWriteSensoryCore` (read-merge-write,
  DatabaseManager.cpp), so the fix is enforced on every wholesale save -- proven by
  `tst_saveintegrity_e2e::scenario10` (in-mem 1 + DB 2 -> save -> reload still 2).
- Card growth on the apply side is a SHARED pure helper `appendDbOnlyTailSamples`
  (SensoryData) that BOTH `SensoryPanel::applyMergedScoresToCurrentSession` and the
  e2e test call -- same anti-drift pattern as `overlayMergedScores` (scenario9).
- DV-15: the three desktop `session_name` derivations (live-save = title-only; two
  imports = "title - tester") forked an imported session's natural key on re-save.
  Unified on `canonicalSensorySessionName(title)=title.trimmed()` at all three
  sites; Detailed panel was already title-only. The DB-side re-key of
  already-forked rows is a separate OWNER-GATED migration (dv15-rekey).
- Postgres least-privilege GOTCHA: `REVOKE EXECUTE ON FUNCTION f() FROM <role>` does
  NOT deny the role if EXECUTE is still granted to PUBLIC (every function defaults
  to PUBLIC EXECUTE, and every role is a PUBLIC member). Verified:
  `has_function_privilege('sensory_web','dve_commit_cell(...)','EXECUTE')` = true
  after a FROM-the-role revoke. To actually lock a generic mutator away from a
  least-priv web role, `REVOKE EXECUTE ... FROM PUBLIC` (the schema owner keeps it).

## 2026-06-29 -- web-form json_data contract + Navigator dock safety net (DATAVIEWER-11/19)

- "the form" is AMBIGUOUS. The owner reported "the assessor field fills both
  assessor and tester in the database" as a bug "in the form", but the WEB form
  (app.py + dve_append_sensory_sample) maps tester/assessor to the correct COLUMNS.
  The real defect was on the DESKTOP READ side, surfaced via the DB Browser. Trace
  the actual data flow end-to-end before assuming which "form" (web vs the desktop
  sensory panel) or which side (write vs read) owns the bug.
- ARCHITECTURAL CONTRACT: for a SensorySession, `json_data` (JSONB) is the COMPLETE
  source of truth -- `DatabaseManager::loadSensorySession` rebuilds the whole struct
  from it via `sensorySessionFromJson`, and the Browser listing reads tester/title
  from `json_data->>'...'`. The dedicated columns (session_name, tester_name,
  assessor_name, media, date) are a DENORMALIZED subset for the natural-key index +
  SELECT-without-parse. A writer that sets ONLY the columns (the web form did:
  json_data='{"samples":[]}') produces rows whose header fields read back EMPTY on
  the desktop -- tester then fell back to the assessor (SensoryPanel sessionLabel),
  i.e. "assessor in both columns". Fix = make the WRITER honor the contract
  (dve_append seeds a full json_data blob mirroring sensorySessionToJson keys) + a
  one-time backfill (dv11c) for rows already written. Don't paper over it by teaching
  every desktop reader to fall back to columns -- fix the writer.
- Qt dock persistence: a QDockWidget with NO post-restoreState() visibility guard
  can vanish for good. Closing the app while a dock is FLOATING makes restoreState()
  restore it hidden/off-screen; unlike m_notesDock (which force-syncs visibility by
  mode), the Navigator had no guard -> "completely missing" with no way back. Give an
  essential dock (a) a deferred startup rescue (re-dock+show if !isVisible() or
  floating && !QGuiApplication::screenAt(frameGeometry().center())) and (b) a
  user-facing master reset (Settings > Panels > Reset Panels / resetPanelLayout()).
- Per-mode ribbon button sets must occupy IDENTICAL positions (owner directive,
  2026-07-06). The Home > File group used to show 3 buttons in TPM mode and 4 in
  the sensory modes, so every button after the gap JUMPED on a mode switch --
  the owner called it out as jarring. Fix was to add the missing slot (TPM Save,
  before Load File) rather than accept the reflow. Rule going forward: when two
  modes swap button sets in the SAME ribbon group, keep the same count, order,
  and (via the shared fixed button width) the same x-positions -- add a
  placeholder action or reorder, never let buttons shift. Same reasoning as the
  Reports tab, which RELABELS one button in place instead of hiding/showing
  different ones.
- Qt QDockWidget: a dock floated PROGRAMMATICALLY (setFloating(true) -- which
  includes restoreState() bringing back a float from the previous session)
  cannot be re-docked by DRAGGING: the panel follows the mouse but Qt never
  shows the drop preview (double-click re-dock still works). Floats the user
  creates by dragging OUT re-dock fine, including after mode switches.
  Verified E2E 2026-07-06 with real mouse drags (computer-use) after a
  headless probe showed setFloating(false) works and constraints were sane --
  the bug is drag-path-only, which no programmatic probe would have caught.
  Fix: never START a session with a floating dock; the restoreSettings()
  rescue re-docks any restored float, so every in-session float is
  drag-created and drag-dockable. If a future feature needs to float a dock
  programmatically, it must accept that drag-redock won't work for it.
- "Scroll right doesn't reveal the end of long navigator names" (owner,
  2026-07-06) had TWO layers. (1) QTreeView's default stretch-last-section
  clamps column 0 to the viewport, so long file/sheet names are cut with a
  ZERO horizontal scroll range -- no amount of scrolling can ever reach the
  tail. Fix: setStretchLastSection(false) + ResizeToContents on column 0 +
  ElideNone. (2) The sensory QListWidget actually scrolled fully (verified by
  probing the REAL MainWindow with injected long items), but the sidebar shows
  TWO stacked horizontal scrollbars: the ScrollHost's panel bar at the very
  bottom (tiny range, pans the whole panel a few px, NEVER reveals list text)
  and the list's own bar above it. A user grabbing "the" scrollbar gets the
  useless one. Mitigation: full-label tooltips on every nav item (tree, list,
  detailed) + per-pixel horizontal scrolling. When a widget lives inside a
  ScrollHost, remember its own scrollbars coexist with the host's -- design so
  the host's bar rarely appears (keep panel minimums <= the dock width).
- Round 2 on "no cut-off text" (owner escalation 2026-07-07: NEVER cut text in
  nav/notes at ANY window size) -- three findings that supersede parts of the
  entry above. (1) THE TRUE ROOT CAUSE of the owner's repro: the dock-width
  constraints only ran on breakpoint CHANGES, never at startup, so a session
  that starts and stays at Standard could keep a saved dock NARROWER than the
  sidebar panel's minimum -- a strip of the panel (with the list's text and
  even its own scrollbar) lived permanently off-dock, reachable only by the
  useless panel-level ScrollHost bar. applyDockWidthConstraints() now also
  runs in the startup rescue, and the Standard docked minimum tracks
  m_sidebarFullPanel->minimumSizeHint().width(). (2) QTreeView::setWordWrap
  does NOT produce wrapped rows on Qt 6.10/Windows style -- the delegate must
  report wrapped heights itself (WrappingPresenceDelegate: sizeHint lays the
  text out at viewport - indent - icon - dots and caps the width hint;
  requires uniformRowHeights(false) + a queued doItemsLayout on
  sectionResized). QListView wraps natively with setWordWrap(true) (+
  ResizeMode Adjust to re-flow on resize); QTableWidget wraps with wordWrap +
  vertical-header ResizeToContents. (3) Verify text-fit changes by GRABBING
  PNGs of the real widgets (temp env-gated probe) -- scrollbar-range metrics
  said the list was fine while the pixels showed the owner's cut.
- "Stop closing Excel" (owner escalation 2026-07-07, THIRD occurrence of this
  class): the root cause was in PRODUCTION code all along, not in any test
  script -- src/ExcelReader.cpp's embedded COM reader used
  win32com.client.Dispatch("Excel.Application"), which ATTACHES to the user's
  already-running Excel, and its excel.Quit() closed THEIR whole session with
  every open workbook. Every app file-load and every tst_excelreader run did
  it whenever Excel was open. Fix: DispatchEx (private instance) + try/finally
  Close/Quit. RULE: any COM automation of Office apps must use DispatchEx and
  must only ever Quit the instance it created; grep for plain Dispatch( when
  Excel "mysteriously closes".
- The wrap-everywhere navigator change (bbe680b) FROZE the app in production
  and was reverted: item-view size hints that depend on the live viewport
  width (WrappingPresenceDelegate) + Adjust-mode relayout + live NOTIFY churn
  (another client updated a sensory session ~1/sec) = relayout feedback that
  hard-froze the UI within seconds of recovery-restoring sessions, while the
  scroll-based build survived the same churn for 2 hours. Keep: content-sized
  tree column (full h-scroll), per-pixel list scroll, ElideNone, tooltips,
  panel-min dock minimum at startup, wrapped property-table rows, wrapped
  QLabel. Do NOT reintroduce viewport-width-dependent sizeHints in item views.
  Also fixed here: recovery's mode switch now goes through switchToMode()
  (driving the ribbon toggle buttons) -- direct toggleSensoryMode(false) left
  the Sensory button visually checked alongside TPM.

- A PlotConfig with autoScale=false must set BOTH axes explicitly. The report
  and GUI Draw Pressure plots set only yMin/yMax and silently inherited the
  struct defaults xMin=0, xMax=1, so every draw-pressure chart rendered its
  data squeezed against the right edge of a 0-1 axis. Fixed with
  PlotEngine::applyDataXRange() (axis spans first..last data point); use it
  for any manual-scale line plot instead of hand-rolling the x range. Also:
  the report table's "Draw Pressure" cell had quietly been a MEDIAN since
  v0.2.0 while everyone read it as an average -- changed to the mean of
  non-zero rows. When a summary stat's name and formula disagree, fix the
  formula or the label; don't leave them drifting.

- The repo has TWO release build trees and the installer only packages ONE:
  installer.iss sources the ROOT release\DataViewer.exe (in-tree build), NOT
  build\release\DataViewer.exe (out-of-tree). The draw-pressure fix was built
  into build\release, the version gate passed (2.7.0 == 2.7.0, versions can't
  catch same-version staleness), and the "fixed" installer shipped the OLD
  exe - the owner installed it and correctly reported nothing changed.
  build_installer.bat now refuses to package an exe older than any src\
  source file (staleness poka-yoke, tested in both directions). For installer
  builds, always rebuild IN-TREE: qmake CONFIG+=release at the repo root +
  mingw32-make -f Makefile.Release. Verify the packaged artifact chain by
  TIMESTAMPS (src < exe < installer), not by version strings.

- Owner axis rules, now codified (2026-07-07): for EVERY plot with an x and y
  axis - x runs from 0 (never the first data point: a point sitting on the y
  axis looks cut off) to the last data point; y is anchored at 0 and tops out
  at the data maximum + 1. ALWAYS. Implemented as
  PlotEngine::applyDataXRange/applyAnchoredYRange + the same rule inside
  autoRange, dual-axis, and renderBarChart; the per-chart floors
  (computeTpmYMax's 7.0/25.0, drawPressureYMax's 2.0) were removed. Do not
  reintroduce special-case axis scaling without an explicit owner ask.
- "Viscosity reads 0" was not a parser regression - the tester typed
  "800kcp" (units, k multiplier) into the numeric B3 cell and
  QVariant::toDouble returns 0 for it. Numeric HEADER cells must be parsed
  tolerantly (ExcelReader::tolerantCellDouble: leading number + k/K x1000).
  When a "worked before" field reads 0, inspect the REAL workbook cell first
  (openpyxl read-only) before hunting the code.

- Release-merge gotcha (v2.7.0): the sprint branch was cut from the v2.6.0
  WRAP COMMIT (fae7fea), not from main's merge commit, so main had diverged
  (sidecar/Mfused/hotfix commits) and a fast-forward was impossible. Before
  tagging a release, check `git log HEAD..main` for surprises; merge main
  into the branch first, verify the incoming diff touches no app code
  (diff --stat MERGE_BASE main), and only then move main + the tag. Also:
  `git fetch . branch:main` fast-forwards main without a checkout - useful
  when an untracked file collision blocks `git checkout main`.

- Ribbon compaction is ONE global flag (RibbonWidget::m_compactMode cascades
  to every tab/group), but the collapse decision used to measure only the
  CURRENT tab's needed width. So at a narrow width the wide Home tab
  compacted to icons while the narrower Reports/Tools/Settings tabs kept
  their labels, and switching tabs visibly flipped the toolbar style (owner
  bug 2026-07-08). Fix: RibbonWidget::fullModeNeededWidth() now returns the
  MAX need across all tabs, so the widest tab governs and the appearance is
  identical on every tab. Rule: when a single shared flag drives N views,
  derive it from the max/union over all N, not the active one.
- Probe gotcha: a QToolButton in Qt::ToolButtonIconOnly mode with a NULL
  QIcon falls back to rendering its (elided) text, so an icon-only ribbon
  probe with empty icons looks like it still shows labels. Give probe
  buttons a real (even dummy) icon before eyeballing compact-mode renders,
  or assert toolButtonStyle()==0 directly.

- TPM v3 Phase 1 (2026-07-23), three lessons from the schema-driven read-path flip.
- Plan-authored test/code is not pre-reviewed code; three defects in code the plan supplied were caught ONLY by the subagent review loop, never flagged by the plan itself.
  The three were: a vacuous header-alias test that asserted nothing meaningful, an asymmetric python-unavailable skip guard that guarded one processFile call but not its twin, and a dead taken[] collision guard in the positional column path.
  Lesson: review code that arrives inside a plan with the same scrutiny as hand-written code; do not assume it is correct just because the plan carried it.
- The coordinator's premise that the old-variant Resistance column is UI-editable was wrong.
  NotesStoryPanel only builds the regime editor when perRowRegime is set, and onStoryCellEdited early-returns for the non-regime case, so the old-template Resistance column is not editable in the UI.
  Lesson: verify a UI-editability claim against the widget CONSTRUCTION path (which editor delegates are actually built), not against the column-index map.
- Practical Windows/Qt-Test notes from this sprint.
  Qt Test binaries emit nothing to stdout under this Git Bash, so always run them as `-o results.txt,txt` and read the file, never rely on piped stdout.
  A qmake DEFINES that embeds a spaced path (e.g. SRCDIR pointing at the worktree) needs `$$shell_quote`, or the build breaks where the space splits the token.

- TPM v3 smoke-fix (2026-07-24), wiring the header-driven inference path into production.
  The routing gate is `SchemaInference::standardFits`: standard 12-wide sheets keep the byte-identical positional path, non-fitting sheets (13-col S26, 8-col UserSim) take the inference path (`inferSchema` -> `SchemaDrivenReader::parseSheet` NameFirst -> `LegacyAdapter::lowerInferredSheet`).
  CORRECTED ASSUMPTION: the guard said "all 9 committed fixtures keep identity, 0 inference-skips". Reality is 8 of 9 - `tests/data/format_a.xlsx` is a genuine 13-wide (Comparison Test / PV1-5 + Clog?) block, the in-repo synthetic analog of the exact S26 bug, so it now correctly routes to inference and its `tst_v3shadow::productionMatchesLegacyParser` row SKIPs (joining pv13 + usersim8). That skip is the fix working, NOT a regression; both parsers previously mangled format_a identically at 12-wide, which is why it used to pass identity. Only genuinely-standard 12-wide fixtures (the other 8) stay on the 0-skip identity path.
  When a "keep-identical" fixture set is asserted, check each fixture's ACTUAL block width against the routing predicate - do not trust a blanket "all N are standard" count.
  A new src dependency ripples into every test .pro that links the caller: `LegacyAdapter` gaining a `GenericSheetProcessor` call forced `SheetProcessors.cpp`/`TpmCalculator.cpp` into `tst_v3model.pro`, and `SchemaInference` calling `TemplateSchema::headerField` forced `TemplateSchema.cpp` (a .cpp whose methods nothing referenced before) into every DataProcessor-linking suite. Link errors, not compile errors, so they only surface at the final `ld` step.
  Write-back safety: three cell-edit paths (`onStoryCellEdited`, `onPropCellChanged`, `onEditHeaders`) all hardcode `sampleIndex*12`, wrong for 13/8-wide blocks; all three now early-return on `SheetResult::fromInferredSchema`. That flag is serialized into the recovery JSON (so a recovered inference file stays write-protected) but is NOT in the Postgres mapping, so a DB reload silently drops it (documented at the RowDeletedBanner reload site, Phase 3 closes it).

- TPM v3 Phase 3a (2026-07-30), the pre-flight audit before the database long-format migration.
  AUDIT FIRST, AND AUDIT THE TOOLS. Three read-only audits (save path, DB surface, live-sync/snapshot) were run before writing a line of Phase 3 code, and all three overturned a design assumption the research plan had carried for months.
  The one worth generalizing: `tests/start-test-postgres.ps1` applied every migration with `-v ON_ERROR_STOP=0` piped to `Out-Null` and no exit check, and its in-file comment justified that by claiming the migrations were idempotent so it merely "tolerates the expected already-exists notices". That justification was false - `IF NOT EXISTS` and `CREATE OR REPLACE` do not raise errors. Two migrations really were failing, and `2026-05-16-cell-focus.sql` had been ROLLING BACK IN FULL on every provisioning run for months, because its `SELECT cron.schedule(...)` sits inside its own `BEGIN..COMMIT` and poisoned the transaction. It went unnoticed only because `init.sql` happens to define the same objects.
  Lesson: when a comment explains why a safety check is disabled, treat the explanation as a claim to falsify, not as context. Establish the empirical baseline before changing the flag.
  Corollary, learned the same day: a "fail loudly" fix is only as good as the drill that proves it. The strict-mode rewrite shipped with a regex whose leading-trivia group backtracked into `--` comments, so a comment merely MENTIONING `SELECT cron.` would silently strip the real DDL beneath it - the exact class of bug the change existed to eliminate, reintroduced by the fix. A `(?>...)` atomic group closed it. And the follow-up commit briefly broke the whole script with a bare `"$Label:"` in a string, because PowerShell parses a colon after a variable name as a drive qualifier; that was caught only because the deliberate-break drill actually re-ran the script end to end.
  ARITY GUARDS THAT COMPILE OUT ARE NOT GUARDS. `OfflineSnapshot`'s `assertColumnArity` was a `Q_ASSERT_X`, i.e. absent from every shipped build, and it was wired for four of nine copy blocks - not for `tests`, `samples`, or `data_rows`, whose bind loops carried the bare literals 13/28/19. It is now a release-active check that aborts the regeneration (safe because the atomic tmp-then-rename promotion leaves the previous snapshot intact), and every loop bound is driven by the checked count. Side effect worth having: because a mismatch now aborts, every existing regen test became a drift detector for the Phase 3 rewrite.
  A ONE-LINE CALL-SITE FIX CAN STILL BE UNTESTABLE. Three of 3a's fixes live in `MainWindow.cpp`, which no test `.pro` links, so the new E2E scenario pins the CONTRACT the fixed call site depends on but cannot catch a regression of the call site itself - revert the line and the test still passes. Say so out loud rather than letting a green suite imply coverage it does not have.

- TPM v3 Phase 3c (2026-07-31), the code review of the open-metric persistence layer.
  `QVariant::toBool()` IS NOT A TYPE TEST. On a string it returns true for everything except `""`, `"0"` and `"false"`, so `MetricDefCache::encodeValue`'s bool branch turned a `did_burn` cell reading "no" into a stored TRUE. Every negative answer inverted, and because the write replaced the source text, it was then stable at the wrong value - strictly worse than the pre-3c behaviour of dropping it. The same shape hid in `numberlist` (`QVariant::toList()` returns an EMPTY list for a non-sequence, so "N/A" was stored as the literal `"[]"`).
  Lesson: a lossy conversion helper is not a predicate. When routing a value into a typed storage slot, ask whether the value can HONESTLY occupy that slot and fall back to the lossless text form when it cannot - never let a coercion function double as the test for whether coercion is appropriate. The unit test that missed this used a real C++ `bool`, i.e. the one input for which the buggy branch is correct.
  A CATALOG QUERY THAT RETURNS NOTHING IS NOT A SAFE DEFAULT. The H1 prune guard read `information_schema.columns`, which is privilege-filtered and schema-hardcoded; an empty result makes `key NOT IN (<empty>)` true for every key, so the guard silently evaporates and everything becomes prunable - the exact failure it exists to prevent. It now reads `pg_attribute` via `to_regclass` (search-path aware, and NULL rather than a raise inside the save transaction) and DECLINES to prune when the lookup comes back empty.
  TWO GUARDS OVER ONE INVARIANT MUST SHARE ONE SOURCE. The writer had no wide-column check while the prune had one, so a key colliding with a wide column was written, was unprunable by construction, and - the read has no key filter - was reloaded and rewritten forever. Both halves now run off a single catalog read taken once per save.
  An availability probe must cover everything the guarded code touches: `MetricDefCache::load()` checked only `metric_defs` while the save transaction goes on to hit `measurements` and `sample_headers`, and `ensureSchema` creates the three independently and best-effort.
