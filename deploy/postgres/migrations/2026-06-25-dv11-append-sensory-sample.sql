...
BEGIN
    ...
    -- Key columns, computed exactly as the desktop live path does. Mirror
    -- combineTesterRound (TesterRound.h): an EMPTY tester takes NO round suffix
    -- (the desktop returns '' before the suffix branch), so a blank tester never
    -- becomes ' R1'. The service requires a non-empty tester, so this is
    -- defense-in-depth for a direct caller.
    IF v_tester <> '' THEN
